/**
 * Card text and numbers. Safe to import from client components — this module
 * holds no game logic, only what a card says and what it costs.
 */
...
export const FEATURES: Feature[] = [
  ...
  { id: "ledger",  n: "Core Ledger",             c: "platform",   t: 12, r: 8,  k: 6,  s: 2, src: "Session 1", b: "Double-entry bookkeeping. Every balance in the bank derives from it." },
  ...
  { id: "micro",      n: "Microservice Migration", c: "platform", t: 14, r: 8, k: 7, s: 3, src: "Session 3", bonus: { rel: 2 },          b: "Break the monolith so two teams stop queueing behind one deploy." },
  { id: "shard",      n: "Database Sharding",      c: "platform", t: 13, r: 7, k: 5, s: 2, src: "Session 2", bonus: { mit: 1 },          b: "Partition the ledger by customer range. Writes stop fighting each other." },
  { id: "pipeline",   n: "Data Pipeline",          c: "platform", t: 12, r: 7, k: 6, s: 2, src: "Session 2", bonus: { rel: 1 },          b: "Streaming ingestion, not a nightly batch and a prayer. Everything downstream gets fresher." },
  { id: "dwh",        n: "Data Warehouse",         c: "platform", t: 10, r: 5, k: 3, s: 2, src: "Session 2", bonus: { mit: 1 },          b: "Dull, cheap, and the only place the regulator's numbers reconcile." },
  { id: "testauto",   n: "Test Automation Suite",  c: "platform", t: 10, r: 6, k: 3, s: 2, src: "Session 6", bonus: { dev: 1, rel: 1 },  b: "A suite that runs on every commit and actually gets read when it fails." },
  { id: "gates",      n: "Code Quality Gates",     c: "platform", t: 9,  r: 5, k: 2, s: 1, src: "Session 6", bonus: { dev: 2 },          b: "Coverage, linting and review rules enforced by the pipeline, not by asking nicely." },
  { id: "errtrack",   n: "Error Tracking",         c: "platform", t: 8,  r: 5, k: 3, s: 1, src: "Session 6", bonus: { mit: 1 },          b: "Exceptions aggregated and attributed instead of scrolling a log." },
  { id: "abtest",     n: "A/B Testing System",     c: "platform", t: 10, r: 8, k: 9, s: 2, src: "Session 6", bonus: { rel: 1 },          b: "Ship to five percent first and find out before everyone sees it." },
  { id: "obs",        n: "Observability Stack",    c: "platform", t: 10, r: 5, k: 3, s: 1, src: "Session 9", bonus: { mit: 2 },          b: "Traces, metrics, structured logs. Nobody asks for it." },
  { id: "svcmon",     n: "Service Monitoring",     c: "platform", t: 8,  r: 5, k: 3, s: 1, src: "Session 9", bonus: { mit: 1 },          b: "Health checks and alerting, so the customer is not your monitoring." },
  { id: "runbooks",   n: "Incident Runbooks",      c: "platform", t: 7,  r: 4, k: 2, s: 1, src: "Session 9", bonus: { mit: 2 },          b: "Written, rehearsed, and findable at three in the morning." },
  { id: "gateway",    n: "API Gateway",            c: "platform", t: 11, r: 7, k: 5, s: 2, src: "Session 3", bonus: { rel: 1 },          b: "One edge for auth, routing and versioning instead of twelve." },
  { id: "containers", n: "Containerization",       c: "platform", t: 10, r: 6, k: 3, s: 2, src: "Session 8", bonus: { dev: 1, rel: 1 },  b: "The same image in dev, staging and production. No more works-on-my-machine." },
  { id: "cache",      n: "Caching Layer",          c: "platform", t: 9,  r: 6, k: 5, s: 1, src: "Session 2", bonus: { rel: 1 },          b: "Balance reads served from memory instead of hammering the core." },
  { id: "lb",         n: "Load Balancer",          c: "platform", t: 8,  r: 5, k: 4, s: 1, src: "Session 9", bonus: { mit: 1 },          b: "Spread the traffic and drain a node without an outage." },
  { id: "cdn",        n: "CDN Integration",        c: "platform", t: 7,  r: 5, k: 6, s: 1, src: "Session 9", bonus: { rel: 1 },          b: "Static assets close to the customer. The app stops feeling slow abroad." },
  { id: "profiling",  n: "Performance Profiling",  c: "platform", t: 9,  r: 6, k: 4, s: 1, src: "Session 2", bonus: { rel: 1 },          b: "Find the query that costs you the p99 instead of guessing at it." },
  { id: "ratelimit",  n: "Rate Limiting",          c: "platform", t: 7,  r: 5, k: 3, s: 1, src: "Session 8", bonus: { mit: 1 },          b: "Per-client quotas on the public API. The cheapest abuse defence there is." },
  { id: "auditlog",   n: "Security Audit Log",     c: "platform", t: 9,  r: 5, k: 3, s: 1, src: "Session 8", bonus: { mit: 1 },          b: "Tamper-evident record of who touched which account. The supervisor will ask." },
  { id: "backup",     n: "Backup & Recovery",      c: "platform", t: 9,  r: 5, k: 2, s: 1, src: "Session 9", bonus: { mit: 1 },          b: "Point-in-time restore that somebody has actually tested this year." },
  { id: "dr",         n: "Disaster Recovery",      c: "platform", t: 12, r: 6, k: 2, s: 2, src: "Session 3", bonus: { mit: 1 },          b: "Second region, tested failover, documented RTO." },
  { id: "wiki",       n: "Documentation Wiki",     c: "platform", t: 6,  r: 4, k: 2, s: 1, src: "Session 7", bonus: { dev: 1 },          b: "Decisions written down where the next person will look for them." },
  { id: "migrate",    n: "Core Banking Migration", c: "platform", t: 16, r: 9, k: 12, s: 3, src: "Session 4", bonus: { dev: 1, rel: 1 }, b: "Off the legacy mainframe. The one everybody postpones." },
];

export const FEATURE_BY_ID: Record<string, Feature> = Object.fromEntries(FEATURES.map((f) => [f.id, f]));

export const PRACTICES: Practice[] = [
  { id: "pair",      n: "Pair Programming",      ip: 2, src: "Session 7", e: "+2 on every Development roll." },
  { id: "tests",     n: "Automated Testing",     ip: 2, src: "Session 6", e: "Clears 1 bug each retro. +3 against dependency incidents." },
  { id: "cicd",      n: "CI/CD Pipeline",        ip: 2, src: "Session 8", e: "+3 on every Release roll." },
  { id: "dod",       n: "Definition of Done",    ip: 3, src: "Session 7", e: "Pushed-through features no longer create bugs." },
  { id: "flags",     n: "Feature Flags",         ip: 2, src: "Session 9", e: "Shelved features still earn 20% of their reward." },
  { id: "modular",   n: "Modular Architecture",  ip: 2, src: "Session 3", e: "Picking 2+ features lowers the combined Dev target by 3." },
  { id: "sre",       n: "Observability & SRE",   ip: 2, src: "Session 9", e: "+2 on Mitigation. Outages no longer hit you harder." },
  { id: "agentic",   n: "Agentic Coding",        ip: 3, src: "Session 5", e: "+3 on Development — but pushing through costs 2 bugs, not 1." },
  { id: "blameless", n: "Blameless Postmortems", ip: 1, src: "Session 9", e: "+3 on Mitigation rolls." },
  { id: "retro",     n: "Retro Culture",         ip: 3, src: "Session 7", e: "+1 on every roll you make." },
  { id: "crossfn",   n: "Cross-Functional Team", ip: 3, src: "Session 7", e: "One more feature slot per sprint." },
  { id: "compdes",   n: "Compliance by Design",  ip: 2, src: "Session 10", e: "Clears 1 audit finding each retro." },
];
...
export interface Phase {
  id: "planning" | "dev" | "incident" | "release" | "market" | "retro";
  n: string;
  lead: string;
  minutes: number;
  note: [string, string];
...
export const PHASES: Phase[] = [
  { id: "planning", n: "Sprint Planning", minutes: 5, lead: "Each group picks features up to its capacity, then commits.",
    note: ["Scope is a commitment, not a wish list", "Every feature you add raises one shared bar. Two features are not two chances — they are one harder roll. This is batch size, and it is the same reason large releases fail more often than small ones."] },
  { id: "dev", n: "Development", minutes: 2, lead: "One d20 per group against the combined target. All or nothing.",
    note: ["Why estimation feels unfair", "Capability is the modifier, not the die. Groups that bought practices early are not luckier — they shifted the whole distribution. Compare the bonus columns at the end."] },
  { id: "incident", n: "Incident", minutes: 2, lead: "One card. It hits every group, but not equally.",
    note: ["Operations is part of engineering", "The card does not care what you planned. What decides the outcome is what you built three sprints ago — observability, failover, monitoring. Session 3 called this the non-functional half of the architecture, and Session 9 is where it is paid for."] },
  { id: "release", n: "Release", minutes: 2, lead: "Built is not shipped. Bugs tax every release roll.",
